...
module.exports.run = async function ({ api, event, args, Users, Threads }) {
  ...
  const apiKey = "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";
  ...
  if (blank.length < 2) {
    if (args.includes("time") || args.includes("oras") || args.includes("panahon")) {
      ...
      api.sendMessage("ano ba yung itatanong mo bhe?", event.threadID, event.messageID);
      api.setMessageReaction("✅", event.messageID, (err) => {}, true);
    }
  } else {
    api.sendMessage("🤗 wait hanapin ko'tong : " + args.join(" "), event.threadID, event.messageID);
    try {
